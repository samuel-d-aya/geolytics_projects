...
class MyymaelaetSpider(scrapy.Spider):
    name = "myymaelat"
    allowed_domains = ["spatial.virtualearth.net"]
    start_urls = [
        "https://spatial.virtualearth.net/REST/v1/data/d5239b243d6b4672810cbd11f82750f5/Filialdaten-FI/Filialdaten-FI?"
    ]
    ...
    def parse(self, response: Response) -> Iterator[Dict[str, Any]]:
        ...
        for store in data:
            ...
            yield {
                ...
                "state": None,
                "website": f"https://www.lidl.fi/s/fi-FI/myymaelaet/{(store.get("Locality", "") or store.get("CityDistrict", "")).replace(" ", "-").lower()}/{store.get("AddressLine", "").split(",")[0].replace(".", "").replace("%", "-").replace(" ", "-").lower()}/",
            }
